# leartech-catalog-mcp — Claude Context
...
> Rename this template from `leartech-catalog-mcp` to
...
for `leartech-catalog-mcp` and substituting. For Go, also update
the module path in every `.go` file's `github.com/mikelear/leartech-catalog-mcp/...` imports.
...
| `charts/leartech-catalog-mcp/` | Helm chart with `leartech-helm-library` dependency |
